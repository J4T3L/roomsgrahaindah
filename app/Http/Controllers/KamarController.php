...
class KamarController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'image2' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:100000',
        'image3' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:100000',
        'image4' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:100000',
        'image5' => 'nullable|image|mimes:jpeg,png,jpg,gif,svg|max:100000',
            'harga' => 'required',
            'kamar'=> 'required',
        ...
        if ($request->file('image2')) {
            $validatedData['image2'] = $request->file('image2')->store('post-images', 'public');
        }

        if ($request->file('image3')) {
            $validatedData['image3'] = $request->file('image3')->store('post-images', 'public');
        }

        if ($request->file('image4')) {
            $validatedData['image4'] = $request->file('image4')->store('post-images', 'public');
        }

        if ($request->file('image5')) {
            $validatedData['image5'] = $request->file('image5')->store('post-images', 'public');
        }
